...
where ai.sex_upon_intake like 'intact%' and ao.sex_upon_outcome in ('Neutered Male', 'Spayed Female')
order by ai.animal_id

- ins 테이블을 기준으로 outs 테이블을 left join
- ai.sex_upon_intake가 intact로 시작하고, ao.sex_upon_outcome에 'Neutered Male' 혹은 'Spayed Female'이 포함되어 있는 row를 조건으로 삼고
- ins 테이블의 animal_id를 기준으로 정렬
...
- 첫번째 풀이와 풀이 방법은 같고 where절만 다르게
- 결국에 중성화를 한 것은 ai.sex_upon_intake와 ao.sex_upon_outcome이 다른 것이기 때문에 !=를 이용
```